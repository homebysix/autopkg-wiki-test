...
def main(_):
    ...
    for processor_name in sorted(processor_names(), key=lambda s: s.lower()):
        ...
        output += "\n"
        output += """<p style="text-align: center; border: 1px solid gray; border-radius: 8px; padding: 10px; background-color: rgb(255, 253, 232);"><em><strong>NOTE: This page is automatically generated by GitHub Actions when a new release is tagged.</strong><br />
Updates to the information on this page should be submitted in pull requests to the AutoPkg repository.<br />Processors are located <a href="https://github.com/autopkg/autopkg/tree/master/Code/autopkglib">here</a>.</em></p>"""
        output += "\n"
